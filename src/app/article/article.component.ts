...
import { ActivatedRoute, RouterModule, type ParamMap } from '@angular/router';
import { toSignal } from '@angular/core/rxjs-interop';
import { map } from 'rxjs/operators';
import type { Posts } from '../posts.interface';
...
export class ArticleComponent {
  ...
  // Table of contents items as shown in the screenshot
  public toc = signal<TocItem[]>([
    { id: 'section-1', title: 'التواصل مع الموضوع' },
    { id: 'section-2', title: 'اختيار العدسة المناسبة' },
    { id: 'section-3', title: 'الإضاءة الطبيعية' },
    { id: 'section-4', title: 'التركيز على العيون' },
    { id: 'section-5', title: 'الخلفية والتكوين' },
    { id: 'section-6', title: 'الخلاصة' },
  ]);
  siteInfos: SiteInfo = {
    name: 'عدسة',
  ...
  category: Categories[] = [
    { name: 'إضاءة', count: 3, color: 'emerald', icon: 'settings' },
    { name: 'بورتريه', count: 3, color: 'purple', icon: 'person' },
    { name: 'مناظر طبيعية', count: 2, color: 'blue', icon: 'view_array' },
    { name: 'تقنيات', count: 5, color: 'orange', icon: 'view_agenda' },
    { name: 'معدات', count: 3, color: 'emerald', icon: 'settings' },
  ];
  ...
  // Dynamically extract the current article based on the URL's route parameter 'slug'

  public currentSlug = toSignal(
    this.route.paramMap.pipe(map((params: ParamMap) => params.get('slug'))),
  );

  public article = computed(() => {
    const slug = this.currentSlug();
    return this.Article.find((item) => item.slug === slug);
  });

  // Dynamically compute related articles excluding the current article
  public relatedArticles = computed(() => {
    const current = this.article();
    if (!current) return this.Article.slice(0, 3);
